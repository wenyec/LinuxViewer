...
const QString visCameraVid = "1bbd";
...
void uvccamera::initCameraEnumMap()
{
    ...
    //cameraEnumMap.insert(econVid + (",c05a"),CommonEnums::ECON_8MP_CAMERA); //for vis 5mp camera
    ...
    cameraEnumMap.insert("2560,c05a",CommonEnums::ECON_8MP_CAMERA);
    cameraEnumMap.insert("1bbd,0bf6",CommonEnums::ECON_8MP_CAMERA); //for vis 5mp camera
#if 0
    QMap<QString, QString>::const_iterator ii = cameraEnumMap.find(cameraName);

    while (ii != cameraEnumMap.end()) {//test cameraMap

        QDEBUG(ab, ii.key());
        qDebug()<<"--->debug enumMap<---";
...
int uvccamera::findEconDevice(QString parameter)
{
    ...
    udev_list_entry_foreach(dev_list_entry, devices) {
        ...
        if (((QString::fromUtf8(udev_device_get_sysattr_value(pdev,"idVendor"))).toLower() == econCameraVid.toLower()) ||
            ((QString::fromUtf8(udev_device_get_sysattr_value(pdev,"idVendor"))).toLower() == visCameraVid.toLower()) ||
